...
Turn search traffic into people who trust Acrid enough to hire it — and, where the article is about markets, into subscribers for **The Acrid Trades Daily**. (Rewritten 2026-09-10: the old mission line said the newsletter was the whole point, which is the 2026-05 trading-first era; the 2026-08-17 thesis makes custom builds the headline lane and BOOT.md makes Acrid's life the story. The learn library is the one surface earning real organic traffic — its next step has to match the reader's intent, not funnel a plumber reading about agent memory into a trading newsletter.) Each article must:
- Teach ONE concept in plain English to a reader who will build or buy it, in ONE of two live lanes: (1) **Builder / AI-tools** (default, ~60%) — an AI / automation / agent-building / creator-tools concept, a tool review/comparison, or the practical AI question a small-business owner searches; (2) **Operator teardown** (~40%) — how Acrid built/runs one of its own real systems, from receipts. The **Trades lane is REFRESH-ONLY since 2026-09-15** (no new trading/investing/markets articles; existing pages may be refreshed when GSC shows them earning). Evidence: GSC 7d ending 09-14 put 13 of 14 clicks on builder/tool-review/agent pages and 1 on trading; that reader cannot buy anything `/hire/` sells. Titles never name the reader ("for Beginners" is a validator failure; edit_article heals it).
...
- Funnel readers two ways: a LANE-MATCHED capture CTA (builder/teardown → `/fleet-files/`; a trades refresh → `/daily-brief`), AND **end every article with the hire funnel line** — a natural closing beat of "we can automate this for you" pointing at `/hire/` (custom builds are the headline revenue lane per the 2026-08-17 operator thesis). The trading dashboard is never pitched as the business.
...
2a. **discover_topics** — PRIMARY topic source (added 2026-06-06, replaced the static-bank picker). Casts a wide net across four free live-demand signals, each best-effort, steered to builder / teardown / small-business demand (trading seeds, subs and HN queries removed 2026-09-15):
   - **Google Suggest** — autocomplete expansions per seed term = what people search now
   - **Hacker News (Algolia)** — recent high-point AI/automation/agent stories
   - **Reddit (multi-sub)** — top/week titles across AI/automation subs (automation, n8n, AI_Agents, ClaudeAI, selfhosted…) and trades-business subs (HVAC, Plumbing, Roofing, Contractor, smallbusiness) = live questions from people who build or buy
   - **Our GSC mirror** — queries we already get impressions for but rank pos >20 (cheap wins)
   Then ONE Sonnet pass clusters + scores the firehose by demand × freshness × conversion-fit (biased by `seeds.json` conversion_anchor toward the newsletter signup / dashboard / relevant affiliates) and writes a ranked `state/topic-queue.json`. Categories: how-to, tools-review, ai-tools, automation, agent-building, creator-tools, operator-teardown, geo, mcp, claude-api (trading categories retired 2026-09-15). Lane balance target ~60% builder / 40% teardown. Steer it by editing `data/seeds.json`, not by hand-listing topics.
2b. **pick_topic** — deterministic, no LLM. Reads `state/topic-queue.json` (PRIMARY); `data/keyword-bank.json` is fallback only. Filters out:
   ...
   - **Lane-quota pass (2026-07-10):** computes the running lane mix from the last 12 published records (`seen_topics.jsonl`; lane from the record's `lane`/`category` field, else keyword-derived from slug+title) and picks the highest-scoring eligible topic from the most UNDER-SERVED lane vs the 60% builder / 40% teardown target. Falls through to the next lane when a lane has no candidates. A net-new topic in a RETIRED lane (`trades`) is never eligible; a REFRESH of a live trading page still is. This is what actually enforces the mandated mix — the queue's own balance is advisory.
...
4. **edit_article** — Sonnet pass. Stages:
   ...
   - `heal_frontmatter_title` — strips a "for beginners" / "beginner guide" tic from the title (slug untouched) before the validator fails on it (2026-09-15).
   - `ensure_newsletter_cta(body, lane)` — guarantees the LANE'S capture CTA if the writer omitted it: `/fleet-files/` for builder/teardown, `/daily-brief` only for a trades refresh (2026-09-15; it used to append the trading newsletter to every article).
